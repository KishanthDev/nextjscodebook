...
class QaService {
  static async saveQA(botId, question, answer, embedding) {
    ...
    const existingIndex = qaPairs.findIndex(q => q.botId === botId && q.question === question);

    if (existingIndex >= 0) {
      qaPairs[existingIndex] = { ...qaPairs[existingIndex], answer, embedding, updatedAt: new Date().toISOString() };
    } else {
      qaPairs.push({ botId, question, answer, embedding, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() });
    }
