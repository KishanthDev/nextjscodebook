                ...
                <form onSubmit={handleFormSubmit} className="space-y-3">
                    ...
                    <div className="flex flex-col items-center justify-center mt-6 space-y-2">
                        <motion.button
                            type="submit"
                            disabled={isSaving}
                            className="rounded-full px-4 py-2 text-sm flex items-center justify-center transition-all duration-300"
                            style={{
                                backgroundColor: settings.submitBtnBgColor || '#3b82f6',
                                color: settings.submitBtnFontColor || '#ffffff',
                                width: isSubmitted ? '56px' : '100%',
                                height: isSubmitted ? '56px' : 'auto',
                            }}
                            animate={{ width: isSubmitted ? '56px' : '100%', height: isSubmitted ? '56px' : 'auto' }}
                        >
                            {isSubmitted ? (
                                <motion.svg
                                    ...
                                    style={{ fill: settings.submitBtnFontColor || '#ffffff' }}
                                >
                                    <path
                                        fillRule="evenodd"
                                ...
                                settings.submitBtnText || 'Submit'
                            )}
                        </motion.button>

                        {isSubmitted && (
                            <motion.div
                                style={{ color: settings.submitBtnFontColor || '#ffffff' }}
                                className="text-sm font-semibold"
                                initial={{ opacity: 0, y: 5 }}
                                animate={{ opacity: 1, y: 0 }}
                                transition={{ delay: 0.3, duration: 0.4 }}
                            >
                                {settings.submitBtnSubmittedText || 'Submitted!'}
                            </motion.div>
                        )}
                    </div>
                </form>
